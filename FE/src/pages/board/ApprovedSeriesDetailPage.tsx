          ...
          <div className="space-y-4 text-sm">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {series.genres.map(g => (
                  <ShadcnBadge key={g} variant="outline">{g}</ShadcnBadge>
                ))}
              </div>
            </div>
            <div>
            ...
            <ul className="space-y-2">
              {schedules.map(s => (
                <li key={s.id} className="flex justify-between gap-3 text-sm border-b border-border pb-2 last:border-0">
                  <span>
                    {formatPublishingIssueLabel(s.issueNumber)} · {s.publishDate}
      ...
      <aside className="shrink-0 w-full lg:w-[min(360px,34vw)] lg:min-h-0 lg:overflow-y-auto border-t lg:border-t-0 lg:border-l border-border p-6 space-y-5 bg-background">
        ...
        <Card>
          ...
          <div className="space-y-3">
            ...
            {latestSchedule && (
              <p className="text-xs text-muted-foreground">
                Lịch gần nhất: {formatPublishingIssueLabel(latestSchedule.issueNumber)} · {latestSchedule.publishDate}
              </p>
            )}
